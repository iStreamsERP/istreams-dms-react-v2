    ...
    return (
        <div>
            ...
            >
                ...
                <span className={cn("text-xs flex-grow", !hasChildren && !useCheckbox && "ml-1")}>
                    {node.label}
                </span>
...
const UserRights = () => {
    ...
    useEffect(() => {
        if (formsList.length > 0) {
            ...
            formsList.forEach(form => {
                ...
                formTypes[form.FORM_TYPE].children.push({ id: form.FORM_NAME, label: form.ROLE_DESCRIPTION || form.FORM_NAME, formData: form, type: 'Form' });
            });
        ...
        if (selectedForms.length > 0) {
            ...
            selectedForms.forEach(form => {
                ...
                if (!moduleMap[MODULE_NAME]) {
                    moduleMap[MODULE_NAME] = { id: `assigned-module-${MODULE_NAME}`, label: MODULE_NAME, children: {} };
                }

                const formTypes = moduleMap[MODULE_NAME].children;
                if (!formTypes[FORM_TYPE]) {
                    formTypes[FORM_TYPE] = { id: `assigned-type-${MODULE_NAME}-${FORM_TYPE}`, label: FORM_TYPE, children: [] };
                }

                formTypes[FORM_TYPE].children.push({ id: form.id, label: form.label, formData: form.formData, type: 'Form' });
            });
    ...
    const fetchUserRoles = async (USER_NAME) => {
        ...
        try {
            const rolesData = { DataModelName: "general_roles_users", WhereCondition: `USER_NAME = '${USER_NAME}'`, Orderby: "ROLE_ID" };
            ...
            const ROLE_ID = userRolesResponse.map(role => `'${role.ROLE_ID}'`).join(',');
            const masterRolesData = { DataModelName: "general_roles_master", WhereCondition: `ROLE_ID IN (${ROLE_ID})`, Orderby: "ROLE_ID" };
            ...
            if (Array.isArray(masterRolesResponse)) {
                const combinedRoles = userRolesResponse.map(userRole => {
                    ...
                    return {
                        ROLE_ID: userRole.ROLE_ID, USER_NAME: userRole.USER_NAME, ROLE_NAME: masterRole?.ROLE_NAME || 'N/A', ROLE_DESCRIPTION: masterRole?.ROLE_DESCRIPTION || 'No description available'
                    };
                });
    ...
    const fetchUserFormAccess = async (USER_NAME) => {
        if (!USER_NAME) {
            setSelectedForms([]);
            return;
        }
        ...
        setLoading(true);
        try {
            const rightRequestData = { DataModelName: "USER_RIGHTS", WhereCondition: `USER_NAME = '${USER_NAME}'`, Orderby: "MODULE_NAME, FORM_NAME" };
            ...
            const formItems = rightsResponse.map(right => ({
                ...
                formData: {
                    MODULE_NAME: right.MODULE_NAME || '', FORM_NAME: right.FORM_NAME, ROLE_DESCRIPTION: right.FORM_NAME, FORM_TYPE: right.FORM_TYPE || ''
                }
            }));
                ...
                if (formNames) {
                    ...
                    if (Array.isArray(formsResponse)) {
                        const updatedFormItems = formItems.map(item => {
                            ...
                            return {
                                ...item,
                                label: formDetails.ROLE_DESCRIPTION || item.label,
                                formData: {
                                    ...item.formData,
                                    ROLE_DESCRIPTION: formDetails.ROLE_DESCRIPTION || item.formData.ROLE_DESCRIPTION,
    ...
    const handleRemoveSelectedForm = async (node) => {
        try {
            ...
            if (userDetails.USER_NAME) {
                try {
                    const deletePayload = { UserName: userData.currentUserLogin, DataModelName: "USER_RIGHTS", WhereCondition: `USER_NAME = '${userDetails.USER_NAME}' AND FORM_NAME = '${node.id}'` };
    ...
    const handleSave = async () => {
        ...
        try {
            const existingResponse = await getDataModelService(
                { DataModelName: "USER_RIGHTS", WhereCondition: `USER_NAME = '${userDetails.USER_NAME}'`, Orderby: "" },
            ...
            for (const form of selectedForms) {
                ...
                const formAccessData = {
                    USER_NAME: userDetails.USER_NAME, MODULE_NAME: form.formData.MODULE_NAME || 'General',
            ...
            toast({ variant: "default", title: "Success", description: `Saved rights items to user ${userDetails.USER_NAME}`, duration: 3000 });
        } catch (error) {
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                                    ...
                                    <CommandGroup>
                                        {loadingUsers ? (
                                            ...
                                            usersList.filter(user =>
                                                user.USER_NAME.toLowerCase().includes(userSearchInput.toLowerCase())
                                            ).map((user) => (
                                                <CommandItem
                                                ...
                                                >
                                                    ...
                                                    <Check className={cn(
                                                        "ml-auto h-4 w-4",
                                                        userDetails.USER_NAME === user.USER_NAME ? "opacity-100" : "opacity-0"
                                                    )} />
                                                </CommandItem>
                                            ))
                                        )}
                        ...
                        <div className="border rounded-md">
                            <Table>
                                <ScrollArea className={userRoles.length > 4 ? "h-[278px]" : "h-auto"}>
                                    <TableHeader className={userRoles.length > 4 ? "sticky top-0 bg-background z-10" : ""}>
                                        <TableRow>
                ...
                {/* Add Access Rights */}
                <Card className={`flex flex-col p-4 ${!isUserSelected ? 'opacity-50 pointer-events-none' : ''}`}>
                    <Label className="text-xs font-semibold mb-2">Add Access Rights</Label>
            ...
            <div className="flex gap-4 justify-end">
                <Button variant="outline" onClick={handleCancel} disabled={!isUserSelected}>
                    Cancel
                </Button>
                <Button
                    onClick={handleSave}
