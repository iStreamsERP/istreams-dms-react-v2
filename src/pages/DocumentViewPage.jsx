...
export default function DocumentViewPage() {
  ...
  }, [getDocMasterListPayload, userData.currentUserLogin, userData.clientURL, processAssignmentData]);
  ...
  const loadMoreDocs = useCallback(() => {
    ...
    const nextBatch = docsData.slice(currentCount, currentCount + LOAD_MORE_COUNT);
  ...
  const filteredDocs = useMemo(() => {
    ...
    return displayedDocs.filter(doc => 
  ...
  const handleEmployeeSelect = useCallback(async (doc, selectedUserName) => {
    setTaskData(prev => ({
      ...prev,
          ...
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-2">
              {loadingDocs && displayedDocs.length === 0 ? (
                ...
                <div className="col-span-full text-center py-8">
                  <p className="text-gray-500">No documents match your search: "{debouncedFilter}"</p>
                  <Button 
            ...
            {showMore && !debouncedFilter && (
              <div className="text-center py-4">
                ...
                >
                  {loadingDocs ? "Loading..." : `Load More (${docsData.length - displayedDocs.length} remaining)`}
                </Button>
              </div>
            )}
            ...
            {displayedDocs.length > 0 && (
              <div className="text-center text-sm text-gray-500 py-2">
                Showing {Math.min(filteredDocs.length, displayedDocs.length)} of {docsData.length} documents
