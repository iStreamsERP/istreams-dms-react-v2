...
export const SignUpStep3 = ({
  ...
  // Initiate verification for secondary contact
  const handleOptionalVerification = async (contact, isEmail) => {
  ...
  const verifyOptionalOtp = async () => {
    ...
    try {
      ...
      if (isEmail) {
        ...
        onOptionalVerify(optionalVerification.contact, true);
      } else {
        await confirmationResult.confirm(otp);
        onOptionalVerify(optionalVerification.contact, false);
      }
      closeModal();
    } catch (error) {
      setOptionalVerification(prev => ({
  ...
  const SignupSchema = Yup.object().shape({
    ...
    // ... rest of your validation schema
      ...
      {({ values, handleChange, setFieldValue }) => (
        <Form className="space-y-6 overflow-y-auto p-1">
          <div className="grid grid-cols-1 gap-4">
            {/* Primary Contact (Always Verified) */}
            ...
            {/* Secondary Contact (Requires Verification) */}
            <div className="flex flex-col">
              <Label>{!isEmailPrimary ? "Email" : "Phone"}</Label>
              <div className="flex gap-2">
                {!isEmailPrimary ? (
                  ...
                  <PhoneInput
                    ...
                    onChange={(phone) => setFieldValue("LOGIN_MOBILE_NO", `+${phone}`)}
                    inputProps={{
